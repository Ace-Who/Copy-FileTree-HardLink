...
:: Menus", that's why there's variable named LiXXX and positional parameters
...
for /f "delims=" %%D in ('dir /s /b /ad "%LiDir%"') do (
  set #Directory=%%D
  call mkdir "%%#Directory:%LiDir%=%Destination%%%" 2>nul
)
:: 方法2
...
for /f "delims=" %%F in ('dir /s /b /a-d "%LiDir%"') do (
  set #File=%%F
  call mklink /H "%%#File:%LiDir%=%Destination%%%" "%%F"
)
